...
# include database connection file
 ...
 # query to select everything based on keywords matched with url keywords
$keywords=explode(',',$viewKeyword);
if(!empty($keywords)){
     $sql  = "SELECT distinct k.keyword, u.url, u.url_title, u.url_description FROM keyword_table k, url_info_table u, make_relation r WHERE k.kw_ID=r.kw_ID AND u.url_ID=r.url_ID AND  k.keyword LIKE '%$keywords[0]%' ";
        if(count($keywords)>1){
        for($i=1;$i<count($keywords);$i++){
         $sql.="OR k.keyword LIKE '%$keywords[$i]%' ";
            }
       }
}
